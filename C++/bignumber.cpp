...
BigNumber BigNumber::divide(BigNumber other) {
	...
	BigNumber b1(this->getString()), b2 = other;
	bool sign = false;
	if (b1.isNegative() && b2.isNegative()) {
		b1.negate();
		b2.negate();
...
BigNumber BigNumber::Mod(BigNumber other)
{
	BigNumber div = *this / other;
	BigNumber res = *this - div * other;
	return res;
}

int64_t BigNumber::get_int()
{
	if (this->digits() > 64)
	{
		throw "The length of string is larger than 64.";
	}
	if (this->isNegative())
	{
	...
	{
		result = result * 10 + (this->_numberString[i] - '0');
	}
	return result;
}
